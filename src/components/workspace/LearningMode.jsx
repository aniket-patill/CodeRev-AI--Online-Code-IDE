...
import { MessageCircle, PanelLeftOpen, PanelRightOpen } from "lucide-react";
...
const LearningMode = () => {
    ...
    // Restore active file for Learning Mode
    useEffect(() => {
        if (activeFiles && activeFiles[MODES.LEARNING]) {
            setSelectedFile(activeFiles[MODES.LEARNING]);
        }
    }, [activeFiles]);

    const handleFileSelect = (file) => {
        setSelectedFile(file);
        updateActiveFile(MODES.LEARNING, file);
    };
                ...
                <PanelGroup direction="horizontal">
                                ...
                                <main className="flex flex-col h-full min-w-0 relative">
                                    ...
                                    <div className="relative z-40 flex items-center justify-between px-6 py-3 border-b border-white/5 bg-zinc-900/30 backdrop-blur-sm shrink-0">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            {!isChatOpen && (
                                                <button onClick={() => setIsChatOpen(true)} className="p-1.5 bg-zinc-900/40 backdrop-blur-sm border border-white/10 rounded-lg text-zinc-400 hover:text-white transition-colors">
                    ...
                    {isChatOpen && (
                        <>
                            ...
                            <Panel defaultSize={25} minSize={20} maxSize={40} collapsible onCollapse={() => setIsChatOpen(false)} order={3} className="bg-zinc-900/40 backdrop-blur-md border-l border-white/5">
                                <Chat
                                    workspaceId={workspaceId}
